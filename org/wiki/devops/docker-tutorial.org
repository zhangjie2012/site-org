...
整体都比较简单，国内注意使用镜像加速器，可选的 Registry migrrors：

- Docker 中国：https://registry.docker-cn.com
- 网易云：http://hub-mirror.c.163.com/
- daocloud：https://www.daocloud.io/mirror， 推荐
...
重启： ~service docker restart~ ，查看信息： ~docker version~ 。
...
Dockerfile 每一行由指令和参数两步分组成：~INSTRUCTION arguments~，指令不缺分大小写，但一般用大写。常用的指令即说明如下：
...
+ ~EXPOSE~ 暴露端口，声明运行时，容器提供服务端口（只是一个声明，运行时并不是因为声明就会开启这个端口的服务），好处是：
    ...
    - ~EXPOSE~ 容器和 ~-p~ 搞混，具体区别看下面的 _端口暴露、-p 和 -P 的区别_
...
+ 容器和虚拟机不同，容器中的应用都应该是以__前台__的方式运行的，容器没有后台的概念。对于容器而言，容器的启动进程就是容器的应用进程，容器就是为了主进程而存在的，主进程退出了，容器就失去了存在意义。所以 ~CMD service nginx start~ 容器执行之后就立刻退出了（因为指令运行结束了）。正确的做法是关闭 nginx 的后台运行，~CMD ["nginx", "-g", "daemon off;"]~；
...
Docker 官方 [Best practices for writing Dockerfiles](https://docs.docker.com/develop/develop-images/dockerfile_best-practices/)。
...
~docker stat~ 用来实时查看容器资源占用统计，~--format~ 参数用来自定义显示格式，~--no-stream~ 禁止不断刷新，只显示一次。具体看[这里](https://docs.docker.com/engine/reference/commandline/stats/#description)。
...
[Docker Hub](https://hub.docker.com/) 是 Docker 官方维护的公共仓库，涵盖了大部分需求。
...
[BusyBox](https://github.com/docker-library/busybox) 是一个很小的镜像库（几兆），集成了一百多个常用的 Linux 命令和工具，方便做快速验证和熟悉 Linux 命令，也可以用来学习 Docker 命令。

** Apline Linux

[Apline Linux](https://alpinelinux.org/) 是基于 ~musi libc~ 和 ~busybox~ 的安全至上的轻量级的 Linux 发行版，而且它提供了自己的包管理工具 [apk](https://pkgs.alpinelinux.org/packages)。Docker 官方已经建议用 Apline 替代之前的 Ubuntu 做为基础镜像环境。
...
1. 镜像运行时，ENTRYPOINT 命令会最先执行；~docker run~ 后执行的命令会追加到 ENTRYPOINT 之后。ENTRYPOINT 会可以指定脚本：

        COPY ./docker-entrypoint.sh /
        ENTRYPOINT ["/docker-entrypoint.sh"]
        CMD ["postgres"]

2. CMD 会在 ENTRYPOINT 之后执行，一个 Dockerfile 只能有一个 CMD 命令，多个命令只会执行最后一个；
...
4. 运行容器设置 alternative arguments 会覆盖 CMD

来自：[Docker ENTRYPOINT & CMD: Dockerfile best practices](https://medium.freecodecamp.org/docker-entrypoint-cmd-dockerfile-best-practices-abc591c30e21)：ENTRYPOINT & CMD

** 端口暴露、-p 和 -P 的区别

有很多资料介绍 ~EXPOSE~ 和 ~-p~ 的区别，比如 [这个](https://blog.lab99.org/post/docker-2016-07-14-faq.html#zen-me-ying-she-su-zhu-duan-kou-dockerfile-zhong-de-expose-he-docker-run-p-you-sha-qu-bie) 还有 [这个](https://stackoverflow.com/questions/22111060/what-is-the-difference-between-expose-and-publish-in-docker)，但使用中只要注意：

+ ~EXPOSE~ 只是设置暴露端口，主机无法访问，只是一个声明（虽然也可以在 ~run~ 时 ~--expose~，但是写在 Dockerfile 中比较好），通常用于写 Dockerfile 的人设定的服务暴露的端口，类似一个规范性的东西，一般和 ~-P~ 选项一起使用，在命令行指定 ~-P~ 之后 ~EXPOSE~ 的端口会被随机映射到宿主机上，宿主机上的端口是动态分配的（保证不冲突）。还因为在构建镜像的时候，你无法决定运行期主机端口如何映射到容器内部，因为你不知道运行容器的机器有哪些端口是可用的。
...
来自：[How do I assign a port mapping to an existing Docker container?](https://stackoverflow.com/questions/19335444/how-do-i-assign-a-port-mapping-to-an-existing-docker-container)

* FAQ

_A: docker 作为 crontab 命令时报错，~Error “The input device is not a TTY”~_
...
- [docker error in crontab: the input device is not a TTY](https://medium.com/@ismailyenigul/docker-error-in-crontab-the-input-device-is-not-a-tty-7280cc42cf19)

* 参考资料

+ [Docker - 从入门到实践](https://yeasy.gitbooks.io/docker_practice)：本文主要以这本书为参考骨架，然后根据其它资料扩展
