...
默认的工作空间是 =$HOME/go= ，需要手动创建一下，如果要设置不同的工作目录，你需要设置GOPATH [fn:1]。

/我记得早版本的 go 是不存在默认的工作空间一说的，现在更简单了/
...
如果提示 ~go: cannot use path@version syntax in GOPATH mode~ ，需要添加: ~GO111MODULE=on go get golang.org/x/tools/cmd/godoc~ .
