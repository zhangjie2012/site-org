...
备注：我常用的环境是 MacOS 和 Ubuntu，MacOS 使用 =brew install emacs= 。
...
*** Emacs 启动命令比 Vim 长怎么办？

打开 =.bashrc= 添加 alias： ~alias em="emacs -nw"~ ，这样就可以使用 ~em~ 打开 Emacs 了。很多人也用 vi alias 成 vim，一个道理。
...
+ =F3= 打开 eshell
+ =F4= 插入当前时间，比如 ~2019-05-31 16:08:58~
+ =F6= 显示当前文件路径，比如 =/Users/JerryZhang/wiki/content/programming/code-env-emacs.md=
+ =M-s l= 打开行号模式，默认我不显示行号列，需要的时候再打开
...
+ [[https://github.com/abo-abo/avy][avy]]，光标快速定位（按照字符、单词、行等），我重新绑定了一下快捷键：
    - =(global-set-key (kbd "<f9>") 'avy-goto-char)=
    - =(global-set-key (kbd "M-g f") 'avy-goto-line)=
+ [[https://github.com/company-mode/company-mode][company]]，模块化补全插件（支持各种语言和后端），[[https://github.com/auto-complete/auto-complete][auto-compete]] 用了几年，后来切换到了 company，auto-complete 没有 company 精准。编码必备插件。
...
+ [[https://www.flycheck.org/en/latest/][Flycheck]]，强大的语法检查框架，主流的编程语言都支持，需要编程语言的 lint 工具配合，比如 eslint、golint。编码必备插件。
...
+ [[https://github.com/abo-abo/swiper][ivy]]，自动完成框架。一般 ido+smex，ivy，[[https://github.com/emacs-helm/helm][helm]] 三者必须选一。还没有 helm 的时候，我一直使用 ido+smex，helm 用了四五年，最近才切换到 ivy。
+ [[https://github.com/bbatsov/projectile][projectile]], 项目管理框架，在一个 Emacs 下管理多个项目.
    - 默认 projectile 是使用 ido 的
    - 使用 ivy 推荐搭配 [[https://github.com/ericdanan/counsel-projectile][counsel-projectile]]
    - 使用 helm 推荐搭配 [[https://github.com/bbatsov/helm-projectile][helm-projectile]]
+ [[https://github.com/abo-abo/ace-window][ace-window]]，使用 ~M-o~ 切换两个窗口，多个窗口使用 ~M-o 1/2/3/4..~ 切换，用 [[https://github.com/deb0ch/emacs-winum][emacs-winum]] 用了很多年，它是使用 ~M + 1/2/3/4...9~ 进行窗口切换，也很好用。唯一的问题是很容易和终端的快捷键冲突。一般的终端（Xshell，linux-ternimal）都是用来切换页签的。
+ [[https://github.com/Fanael/rainbow-delimiters][rainbow-delimiters]]，彩虹括号，相同层级的括号颜色一样，不同层级的不同。这个很好玩；
+ [[https://github.com/magit/magit][magit]]，Git 版本管理（因为启动速度慢，所以就不用了）
+ [[https://github.com/magnars/multiple-cursors.el][multiple-cursors]]：多鼠标操作，太帅了。可以看这个视频： [[http://emacsrocks.com/e13.html][Emacs Rocks! Episode 13: multiple-cursors]]

上面是通用的插件，下面是语言编码环境相关的插件：

+ 前端开发：
    - [[https://github.com/felipeochoa/rjsx-mode][rjsx-mode]]：js2-mode 的升级版，js/jsx mode，除了有点点卡，用来挺爽的；
    - [[http://web-mode.org/][web-mode]]：html、xml、css/less，jsx 模式有一个很蛋疼的 bug，参见我提的 [[https://github.com/fxbois/web-mode/issues/1041][issue]]，作者一直没有解决，后来就用 rjsx 了；
+ Python：[emacs-jedi](https://github.com/tkf/emacs-jedi) + [company-jedi](https://github.com/syohex/emacs-company-jedi)，jedi 是 Python 的自动补全和静态分析的库，是个后端服务。emacs-jedi 是 jedi 针对 Emacs 的插件，Vim 也有 [[https://github.com/davidhalter/jedi-vim][vim-jedi]]。
    - 如果 Python 的第三方库跳转和补全有问题时，需要配置 ~jedi:server-arg~ 的 ~--sys-path~ 到你的 Python ~site-packages~ 目录
+ Go：[[https://github.com/mdempsky/gocode][mdempsky/gocode]] 是 Go 的自动补全守护进程，这个项目简直太曲折了，我也躺了很多的坑。目前已经迁移了三次，[[https://github.com/stamblerre/gocode][stamblerre/gocode]]是最新的，支持最新的 Modules 特性，但是太卡了。还是推荐使用 mdempsky/gocode，虽然补全有不实时的问题，但至少很稳定。Emacs 使用 [[https://github.com/dominikh/go-mode.el][go-mode]] 就行了。
    - Go 的版本更新实在太快了，每次变更都比较大，导致目前的工具链看似完整，实则问题多多，有很多兼容问题。

#+BEGIN_QUOTE
语言的开发环境配置一直很恼人（很费时间），我记得以前刚配置 C/C++ 的开发环境时，折腾了一个月左右时间才找到一个相对比较满意的开发环境（折腾完之后使用起来可真爽啊）：`xcscope + etags + c++-mode`。写 Python 的时候也折腾了长时间的缩进问题。Go 就更不用说了···。后来看到了 [LSP（Language Server Protocol）](https://langserver.org/) 项目，感觉这个项目才是终极解法：插件化，C/S 模式。折腾 Go 得时候，试过 [emacs-lsp/lsp-mode](https://github.com/emacs-lsp/lsp-mode)，稳定性还是很大的问题，不怎么敢用，不过可以持续关注这个项目。
#+END_QUOTE
...
如果发现类似问题，可以注释掉其中的两个，只保留一个，比如 `melpta-stable`，一般能解决此问题。之前在安装 helm 的时候经常遇到依赖有问题，刚才（2019-05-31）安装 ivy 也遇到了类似的问题，给 github ivy 提了一个 [issue](https://github.com/abo-abo/swiper/issues/2087) 才解决了这个问题（社区的力量真强大）。

*** Emacs 启动速度太慢怎么办？

`M-x emacs-init-time` 可以查看 Emacs 启动耗费时间。

多一个插件都会增加启动成本，不信你 `emacs -Q` 试试，所以要尽可能的减少插件。你可以使用 [keyfreq](https://github.com/dacap/keyfreq) 来查看你常用的快捷键有哪些。筛选出不常用的插件给干掉，这是解决启动速度慢的根本办法。
...
- 使用 [use-package](https://github.com/jwiegley/use-package)，use-package 并不是包管理工具，只是一个宏，用来配置和加载包。
...
1. 安装依赖库：`pip install jedi virtualenv epc flake8`
1. 如果第三方库找不到定义，找到第三方库的安装路径，然后修改 `init-python` 中的 `jedi:server-args` 即可

前端(JavaScript)：

1. 安装 eslint：`npm install eslint -g`
...
+ `C-c p p`：切换项目
+ `C-c p f`：在打开的项目中打开文件
+ `C-c p s g`：当前项目下面下搜索，使用 grep 命令，不过我更喜欢用 ag => `C-c p s s`
...
+ `C-x C-f`：打开文件（相对于当前的文件目录），Emacs 自带
...
[Magit](https://magit.vc/) 是最出名的一个插件，也是功能最完善的：只要记得 `C-x g` 一个快捷键就行了，然后按 `?` 引导操作即可。

** Resource

*** Blog/Site/Tutorial

+ [Planet Emacslife](https://planet.emacslife.com/)：Emacs 百科全书，大杂烩
+ [Emacs reddit channel](https://www.reddit.com/r/emacs/)：reddit 上的 Emacs 专题
+ [\(or emacs](http://oremacs.com/)
+ [Practical Emacs Tutorial](http://ergoemacs.org/emacs/emacs.html)：Emacs 练习手册
+ [Emacs Themes](https://emacsthemes.com/)：主题集合
+ [awesome-emacs](https://github.com/emacs-tw/awesome-emacs)
+ [MELPA](http://melpa.org/)：插件集合
+ [Living an awesome Life](https://sachachua.com/blog/category/emacs/)：Emacs category
