...
- 控制器(controller):
  + [[file:kubernetes/controller-replicaset.org][ReplicaSet]]
  + ReplicationController: ReplicaSet 是下一代的 RC（支持新的基于集合的标签选择器），建议使用 RS
  + [[file:kubernetes/controller-deployments.org][Deployments]]
  + [[file:kubernetes/controller-statefulsets.org][StatefulSets]]
  + DaemonSet
  + Garbage Collection
